...
<template>
    <div class="own-space">
                  ...
                  >
                      ...
                      <FormItem label="手机号：">
                          ...
                          <div style="display:inline-block;margin-left:20px;font-size:13px;">
                              <a @click="showChangephone">修改手机号</a>
                          </div>
                      </FormItem>
                      <FormItem label="邮箱：">
        ...
        <Modal title="修改手机号" v-model="editphoneVisible" :closable='false' :mask-closable=false :width="500">
            <Form ref="phoneEditForm" :model="phoneEditForm" :label-width="60" :rules="phoneEditValidate">
                <FormItem label="手机号" prop="phone">
                    <Input v-model="phoneEditForm.phone" @on-change="hasChangePhone" placeholder="请输入新手机号"/>
                </FormItem>
                <FormItem label="验证码" prop="code" :error="codeError">
                    <Input v-model="phoneEditForm.code" placeholder="请输入您收到的短信验证码" style="width:200px;margin-right:5px;"/>
...
export default {
  ...
  data() {
    ...
    return {
      ...
      editphoneVisible: false, // 显示填写验证码box
  ...
  methods: {
    init() {
      ...
      this.phoneEditForm.phone = userInfo.phone;
    ...
    handleSuccess(res, file) {
      if (res.message ===  'success') {
        ...
        this.defaultList[0].url = res.data;
    ...
    getIdentifyCode() {
      ...
      this.$refs["phoneEditForm"].validate(valid => {
        if (valid) {
    ...
    submitCode() {
      ...
      this.$refs["phoneEditForm"].validate(valid => {
        if (valid) {
          if (this.phoneEditForm.code.length === 0) {
    ...
    hasChangePhone() {
      if (this.phoneEditForm.phone === this.initPhone) {
        this.canGetIdentifyCode = true;
      } else {
        this.$refs["phoneEditForm"].validate(valid => {
          if (valid) {
